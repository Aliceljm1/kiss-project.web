...
namespace Kiss.Web.Auth.OpenId {
	...
	public class XriDiscoveryProxyService : IIdentifierDiscoveryService {
		...
		private static Uri GetXrdsUrl(XriIdentifier identifier) {
            ErrorUtilities.VerifyProtocol(AuthSection.Configuration.OpenId.XriResolver.Enabled, OpenIdStrings.XriResolutionDisabled);
			...
			return new Uri(
				string.Format(
					...
					xriResolverProxy,
					identifier,
                    AuthSection.Configuration.OpenId.XriResolver.Proxy.Name));
		}
	}
}
